...
struct TrameXbee * getTrame(int * usedXbee){
	...
	uint16_t tailleData = ((uint16_t)(bufferHeader[1] << 8)) | ((uint16_t)bufferHeader[2]);

	 trameRetour = malloc(sizeof(struct TrameXbee) + (trameRetour->header.taille +1)*sizeof(uint8_t));
